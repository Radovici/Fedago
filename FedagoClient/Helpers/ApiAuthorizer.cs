...
namespace DataBazrPeer.Helpers
{
    public class ApiAuthorizer
    {
        ...
        public string UpdateApiClientCallbackUrl(string callbackUrl)
        {
            string requestUriString = string.Format("{0}/api/client/{1}", _serverUrl, _clientId);
            var httpWebRequest = (HttpWebRequest)WebRequest.Create(requestUriString);
            httpWebRequest.Method = "PUT";
            httpWebRequest.ContentType = "application/x-www-form-urlencoded";

            string data = string.Format("callbackUrl={0}", callbackUrl);
